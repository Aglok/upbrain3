    ...
    <div>
        ...
        <v-tabs-items v-model="subject.tabs">
            <v-tab-item v-for="(value, subject, index) in this.$store.state.app.user.subjects" :key="index">
                <v-expansion-panel>
                    <v-expansion-panel-content v-for="(mission, index) in missions(subject)" :key="index">
                        <template v-slot:actions>
                            <v-icon v-if="mission.pivot.done ==='y'" class="icon icon-check"></v-icon>
                        ...
                        <v-card>
                            ...
                            <router-link :to="{ name: 'Table Tasks', params: {mission_id: mission.id}}" class="text-md-center text-xs-center btn-tool black--text">
                                К задачам
                            </router-link>
                        </v-card>
    ...
    export default {
        ...
        methods:{
            ...
            progress(mission, subject){
                return this.$store.state.app.user.subjects[subject].progresses.find(p => p.id === mission.id);
            },
            ...
            artifacts(mission_id, subject){
                ...
                for (let key in missions_artifacts){

                    let artifact = missions_artifacts[key].find(x => x.pivot.mission_id === mission_id);
                    if(artifact)
                        mission_artifacts.push(artifact);
                }

                return mission_artifacts;
            },
            mounted() {
